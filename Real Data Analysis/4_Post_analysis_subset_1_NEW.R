...
posterior.mean.NonSparse.All <- read.table("Output/Output_REAL_B_NonSparse_NumIter_1000_burnin_100.csv", 
                                           header = FALSE, sep = ",")
posterior.mean.All <- read.table(paste0("Output/Output_conf_",round(conf*100),"_REAL_B_NumIter_1000_burnin_100.csv"),
                             header = FALSE, sep = ",")  # dim = 287 X 10 (p x q) 
non.zero.locations.All <- read.table(paste0("Output/Output_conf_",round(conf*100),"_REAL_NonZeroLocation_NumIter_1000_burnin_100.csv"),
                                 header = FALSE, sep = ",")
p.values.All <- read.table("Output/Output_REAL_pValues_NumIter_1000_burnin_100.csv",
                             header = FALSE, sep = ",")
...
write.table(X.selected.names.set.modified,"Output/TOP_15_Subset_1.csv", sep = ',', 
...
ggp
ggsave("Output/Subset_1.png", plot = ggp, width = 12, height = 7, dpi = 400)

write.table(sorted.relevance.indices, "Output/MasterRanks_subset1.csv", sep = ',', 
            row.names = FALSE, col.names = FALSE)
...
write.csv(pval_mat, "Output/Selected_pvalues_subset1.csv", row.names = TRUE)
...
write.csv(sign_mat, "Output/Selected_signs_subset1.csv", row.names = TRUE)
