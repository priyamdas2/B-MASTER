...
X_raw = csvread("Data/X_healthy.csv");

RandSeed = 1;
tic;
filename = ['Data/Y_healthy.csv'];
...
filename = ['Output_HEALTHY_vs_CRC/HEALTHY_BMASTER_RAW_MCMC_NumIter_', ...
    num2str(NumIter), ...
    '_burnin_', ...
    num2str(BurnIn), ...
    '.mat'];
...
if ~isfile(filename)
    ...
    filename = ['Output_HEALTHY_vs_CRC/Output_HEALTHY_REAL_compTime.csv'];
...
for cc = 1:length(conf_list)
    ...
    filename = ['Output_HEALTHY_vs_CRC/Output_conf_',num2str(round(conf*100)),'_HEALTHY_REAL_B_NumIter_',num2str(NumIter),'_burnin_',num2str(BurnIn),'.csv'];
    csvwrite(filename,B_est)
    
    filename = ['Output_HEALTHY_vs_CRC/Output_conf_',num2str(round(conf*100)),'_HEALTHY_REAL_NonZeroLocation_NumIter_',num2str(NumIter),'_burnin_',num2str(BurnIn),'.csv'];
    csvwrite(filename,IndicatorNonzero_est)
    
    filename = ['Output_HEALTHY_vs_CRC/Output_conf_',num2str(round(conf*100)),'_HEALTHY_REAL_MasterRanks_NumIter_',num2str(NumIter),'_burnin_',num2str(BurnIn),'.csv'];
    csvwrite(filename,MasterRanks)
    
end

filename = ['Output_HEALTHY_vs_CRC/Output_HEALTHY_REAL_B_NonSparse_NumIter_',num2str(NumIter),'_burnin_',num2str(BurnIn),'.csv'];
csvwrite(filename,B_post_mean)

filename = ['Output_HEALTHY_vs_CRC/Output_HEALTHY_REAL_pValues_NumIter_',num2str(NumIter),'_burnin_',num2str(BurnIn),'.csv'];
