...
write.table(X.selected.names.set.modified,"Output/TOP_15_Subset_2.csv", sep = ',', 
...
ggsave("Output/Subset_2.png", plot = ggp, width = 12, height = 7, dpi = 400)

write.table(sorted.relevance.indices, "Output/MasterRanks_subset2.csv", sep = ',', 
            row.names = FALSE, col.names = FALSE)
...
write.csv(pval_mat, "Output/Selected_pvalues_subset2.csv", row.names = TRUE)
...
write.csv(sign_mat, "Output/Selected_signs_subset2.csv", row.names = TRUE)
