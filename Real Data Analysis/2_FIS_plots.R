...
NonZeroLocation <- as.matrix(
  read.table(paste0("Output/Output_conf_",round(conf*100),"_REAL_NonZeroLocation_NumIter_1000_burnin_100.csv"),,
             header = FALSE, sep = ",")
)
...
ggsave(paste0("Output/FIS_top", tophowmanynfis, "_wide.png"),
...
if (!exists("Signs")) {
  Signs <- sign(as.matrix(
    read.table("Output/Output_REAL_B_NonSparse_NumIter_1000_burnin_100.csv",
               header = FALSE, sep = ",")
  ))
}
...
write.table(rownames(Mat.selected), "Output/TOP_50_master_predictors_FIS.csv",
...
ggsave("Output/Top50genera_heat_FIS.jpeg", plot = whole_heat, width = 10, height = 6, dpi = 400)
