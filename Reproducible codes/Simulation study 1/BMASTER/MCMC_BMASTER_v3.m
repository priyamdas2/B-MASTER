function [B_values,lambda1square_values,lambda2values, ErrorCumsum] = MCMC_BMASTER_v3(NumIter,RandSeed,DisplayExtraUpdates,X,Y,C,shape_a1,delta_1,shape_a2,delta_2,...
...
for iter = 1:NumIter
    ...
    if(iter > 1)
        ...
        if(DisplayExtraUpdates == 1)
            ...
            fprintf('\n');
            fprintf('=> tauSquareMat(P,Q) last iter: %.4f',tauSquareMat(P,Q));
            fprintf('\n');
            fprintf('=> gammapSquareArray(P) last iter: %.4f',gammapSquareArray(i));
            fprintf('\n');
        end
    ...
    for i = 1:P
        for j = 1:Q
            if(C(i,j) ~= 0)
                ...
                DArrayMat(i,j) = max(10^(-10), min(temp_1,10^10));   % to avoid error
            end
        end
    end
    ...
    for j = 1:Q
        ...
        inv_part = inv(XprodX + D_j + 10^(-10)*eye(P));
    ...
    for i = 1:P
        for j = 1:Q
            if(abs(B(i,j)) > 10^(-2))
                tauInvGauss_a = sqrt(lambda1square*sigmaqSquareArray(j)/((B(i,j)+ 10^(-10))^2));
                pd = makedist('InverseGaussian','mu',tauInvGauss_a,'lambda',tauInvGauss_b + 10^(-10));  % mean = mu
    ...
    for i = 1:P
        if(norm(B(i,:)) > Q*10^(-2))
            ...
            gammapSquareArray(i) = 1/max(10^(-6), random(pd2)); % increase random(pd2)
        else
            mg = Q;
            inv_gamma_scale = lambda2square/2;
            pd2_alt = gamrnd((mg+1)/2, 1/inv_gamma_scale);
            gammapSquareArray(i) = 1/max(10^(-6),pd2_alt);
        end   
    end
