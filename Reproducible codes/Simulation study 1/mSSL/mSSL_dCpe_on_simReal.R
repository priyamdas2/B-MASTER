...
for (dataset_no in 1:NumExp) {
  ...
  write.table(unique.FPR.TPR.sorted, paste0('Output_simReal_AUC_curve_mSSL_dCpe_rep_',
                                            dataset_no,'.csv'), sep = ',', 
              row.names = FALSE, col.names = FALSE)
  
  write.table(t(output), paste0('Output_simReal_TPR_FPR_MCC_AUC_AUC20_sparsity_mSSL_dCpe_rep_',
                                dataset_no,'.csv'), sep = ',', 
              row.names = FALSE, col.names = FALSE)
  write.table(time.taken[3], paste0('Output_compTime_mSSL_dCpe_rep_',
                                    dataset_no,'.csv'), sep = ',', 
              row.names = FALSE, col.names = FALSE)
  write.table(MasterRanks, paste0('Output_MasterRanks_mSSL_dCpe_rep_',
                                  dataset_no,'.csv'), sep = ',', 
              row.names = FALSE, col.names = FALSE)
}