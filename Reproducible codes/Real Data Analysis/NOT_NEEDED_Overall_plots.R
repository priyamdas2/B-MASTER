...
whole_heat <- ggplot(long_data, aes(x = Column, y = Row, fill = as.factor(Value))) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(size = 3.6, face = "bold", angle = 90, hjust = 1),  # Hide x-axis labels
...
ggsave("Top50genera_heat_v2.jpeg", plot = whole_heat, width = 14, height = 5, dpi = 400)
####### Histogram plot #########################################################
