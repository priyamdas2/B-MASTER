...
for (dataset_no in 1:NumExp) {
  ...
  # X <- X[1:100, 1:100]
  # Y <- Y[1:100, 1:100]
  # nonzero.locations <- nonzero.locations[1:100, 1:100]
  ...
  write.table(unique.FPR.TPR.sorted, paste0('Output_simReal_AUC_curve_remMapBic_rep_',
                                            dataset_no,'.csv'), sep = ',', 
              row.names = FALSE, col.names = FALSE)
  
  write.table(t(output), paste0('Output_simReal_TPR_FPR_MCC_AUC_AUC20_sparsity_remMapBic_rep_',
                                dataset_no,'.csv'), sep = ',', 
              row.names = FALSE, col.names = FALSE)
  write.table(time.taken[3], paste0('Output_compTime_remMapBic_rep_',
                                    dataset_no,'.csv'), sep = ',', 
              row.names = FALSE, col.names = FALSE)
  write.table(MasterRanks, paste0('Output_MasterRanks_remMapBic_rep_',
                                  dataset_no,'.csv'), sep = ',', 
              row.names = FALSE, col.names = FALSE)
}
